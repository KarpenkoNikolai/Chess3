	...
	namespace AlphaBeta {
		...
		private:
				...
				int8_t depth, int alpha, int beta, int moveOrder = 0)
			{
				...
				if (moveOrder < 100 && depth < (nodePtr.IsNull() ? 10 : 4) && !pvNode && std::abs(alpha) < (MatVal - 100)) {
				...
				for (uint8_t m = 0; m < searchSize; m++) {
					...
					if (reduce) {
						const int endgamePhase = getEndgamePhase(next);
						
						// Konservativere LMR im Endspiel (weniger Reduktion)
						float lmrBase = 0.5f - (endgamePhase * 0.15f / 256.0f); // 0.35-0.5
						int reduction = int(std::log2(depth) * lmrBase + std::log2(m) * lmrBase + 1.5f);
						
						if (reduction && pvNode) reduction--;
						if (reduction && order > 100) reduction--;
						if (reduction && order > 2000) reduction--;
						if (reduction && endgamePhase > 200) reduction--; // Extra-Reduktion im späten Endspiel
